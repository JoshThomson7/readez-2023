...
// Shop options
$wc_custom_shop = true; // Custom shop - false for standard Woo
...
<section class="wc__wrapper">
    <?php
        if($wc_sidebar) {
            require_once(FL1_WOO_PATH.'templates/woo-sidebar.php');
        }
    ?>
    ...
    <div class="wc__content<?php if($wc_sidebar && (is_shop() || is_tax(array('product_cat', 'product_tag'))  || is_search()) ) { echo ' wc__has__sidebar'; } ?>">
        ...
        <?php
            if(is_shop() && $wc_custom_shop) {
                require_once(FL1_WOO_PATH.'templates/woo-shop.php');

            } else {
                if( $wc_custom_archive && (is_shop() || is_tax(array('product_cat', 'product_tag'))) ) {
                    require_once(FL1_WOO_PATH.'templates/woo-archive-product.php');
                } else {
                    woocommerce_content();
                }
            }
